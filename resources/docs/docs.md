...
The `home-routes` handler in the `kaiden-player.routes.home` namespace
...
The `home-page` function will in turn call the `kaiden-player.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/kaiden_player/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `kaiden-player.handler` namespace:
...
Request middleware functions are located under the `kaiden-player.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `kaiden-player.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `kaiden-player.db.core` in some other namespace.
